...
var map = new Array(64 * 64 * 64); // 262144
var texmap = new Array(16 * 16 * 3 * 16); //12288

function init() {
    for ( var i = 1; i < 16; i++) {
        var br = 255 - ((Math.random() * 96) | 0);
        //generate the block textures
        for ( var y = 0; y < 16 * 3; y++) {
            for ( var x = 0; x < 16; x++) {
                var color = 0x966C4A; //light brown
                if (i == 4)
                    color = 0x7F7F7F; //light gray
                if (i != 4 || ((Math.random() * 3) | 0) == 0) {
                    br = 255 - ((Math.random() * 96) | 0);
                }
                if ((i == 1 && y < (((x * x * 3 + x * 81) >> 2) & 3) + 18)) {
                    color = 0x6AAA40; //dark green
                } else if ((i == 1 && y < (((x * x * 3 + x * 81) >> 2) & 3) + 19)) {
                    br = br * 2 / 3;
                }
                if (i == 7) {
                    color = 0x675231; //dark brown
                    if (x > 0 && x < 15
                            && ((y > 0 && y < 15) || (y > 32 && y < 47))) {
                        color = 0xBC9862; //tan
                        var xd = (x - 7);
...
function renderMinecraft() {
    ...
    var xRot = Math.sin(Date.now() % 10000 / 10000 * Math.PI * 2) * vertical_perspective
            + Math.PI / 2;
    var yRot = Math.cos(Date.now() % 10000 / 10000 * Math.PI * 2) *  horizonal_perspective;
    var yCos = Math.cos(yRot);
    var ySin = Math.sin(yRot);
    var xCos = Math.cos(xRot);
    var xSin = Math.sin(xRot);
    
    var forward_speed = 64;

    var ox = 32.5 + Date.now() % 10000 / 10000 * forward_speed; //forward movement
    var oy = 32.5;
    var oz = 32.5;
